...
Connection.prototype._setupSocket = function (options) {
  ...
  var inject = reconnect(function (opts) {
    ...
    if (opts.preserve === 'on') {
      ...
      opts = previousOptions = opts || previousOptions   // Параметры теряются
    }
...
Connection.prototype.openStream = function () {
  var attrs = {}
  for (var k in this.xmlns) {           // Здесь возможно создаётся XML-файл
  ...
  if (el.name === 'stream:stream') {
    ...
    el.t(' ')
    var s = el.toString()     //Здесь создаётся самый первый xmlns-запрос
