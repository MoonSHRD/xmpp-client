...
var sign = require('./authentication/sign')
...
function Client (options) {
  ...
  this.availableSaslMechanisms = [
    XOAuth2, External, DigestMD5, Plain, Anonymous, sign
  ]
...
Client.prototype._useStandardConnect = function () {
  ...
  if (this.options.preferred) {
    ...
    this.preferredSaslMechanism = 'SIGN'  // механизм авторизации по умолчанию
  }
...
Client.prototype._handleAuthState = function (stanza) {
    switch (stanza.name) {
        case 'challenge':
            if (stanza.attrs.xmlns==NS_XMPP_SASL) {
                var challengeMsg = decode64(stanza.getText())
                var responseMsg = (this.mech.challenge(stanza.children[0]))
                var response = new Element('response', {xmlns: NS_XMPP_SASL}).t(responseMsg)
                this.send(response)
            } else if (stanza.attrs.xmlns==NS_AUTH) {
                var challengeMsg = decode64(stanza.getText())
                var responseMsg = (this.mech.challenge(stanza.children[0]))
                var response = new Element('response', {xmlns: NS_XMPP_SASL}).t(responseMsg)
            }
...
Client.prototype._handlePreAuthState = function () {
  ...
  //   this.mech.authzid = this.jid.bare().toString();
    ...
    this.mech.new_nickname(this);
    // this.send(new Element('username', this.mech.username);
